...
const volumeCache = new Map<string, Cached<Volume>>();
const indexCache = new Map<string, Cached<ChapterMeta[]>>();
const metadataCache = new Map<string, StoryMetadata>();
const volFileCache = new Map<string, string[]>();
const MAX_VOLUME_CACHE_SIZE = 10;

function jsonMtime(filePath: string): number {
  const gzPath = filePath + ".gz";
  try {
    return fs.statSync(/* turbopackIgnore: true */ gzPath).mtimeMs;
  } catch {
    try {
...
function readJsonAny<T>(filePath: string): T | null {
  try {
    const gzPath = filePath + ".gz";
    const buf = fs.readFileSync(/* turbopackIgnore: true */ gzPath);
...
function existsAny(filePath: string): boolean {
  ...
  return (
    ...
    fs.existsSync(/* turbopackIgnore: true */ gzPath)
  );
}
...
export function makeDataDir(baseDir: string) {
  ...
  function listStories(): string[] {
    ...
    return fs
      .readdirSync(dataDir)
      .filter((d) => {
        const storyDir = path.join(/* turbopackIgnore: true */ dataDir, d);
  ...
  function getChapterIndex(slug: string): ChapterMeta[] | null {
    ...
    const mtime = jsonMtime(filePath);
    ...
    if (data) {
      indexCache.set(filePath, { value: data, mtime });
    }
    return data;
  }
    ...
    for (let delta = 0; delta <= 2; delta++) {
      ...
      for (const tryVol of candidates) {
        ...
        const logicalPath = path
          .join(/* turbopackIgnore: true */ storyDir, volFile)
          .replace(/\.gz$/, "");
        
        const mtime = jsonMtime(logicalPath);
        let cached = volumeCache.get(logicalPath);
        if (!cached || cached.mtime !== mtime) {
          ...
          if (volData) {
            ...
            cached = { value: volData, mtime };
            volumeCache.set(logicalPath, cached);
          } else {
            cached = undefined;
          }
        }
        const volData = cached?.value;
...
function makeAllSources() {
  ...
  const sources = fs
    .readdirSync(baseDir)
    .filter((d) => d !== "iqiyi" && fs.statSync(path.join(baseDir, d)).isDirectory());
